...
- **Diagrams:** [KubeDiagrams](https://github.com/philippemerle/KubeDiagrams)
  ...
  (when the Helm chart / K8s operator packaging lands), and
  opportunistically earlier via the `kubectl diagrams` plugin to
  capture live cluster topologies for milestone reports and incident
  write-ups. SVGs land in `docs/diagrams/`, regenerated by CI.
